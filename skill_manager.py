...
class SkillManager:
    ...
    def get_skill_paths(self):
        """Get virtual paths for all skills (for DeepAgents)"""
        paths = []
        for skill in self.skills:
            # Skip if skill is not a dict
            if not isinstance(skill, dict):
                continue
            skill_name = skill.get("name", "unknown")
            paths.append(f"/skills/{skill_name}/")
        return paths

    def get_skills_files_dict(self):
        """Get files dictionary for DeepAgents (virtual path -> content)"""
        files_dict = {}
        for skill in self.skills:
            # Skip if skill is not a dict
            if not isinstance(skill, dict):
            ...
            skill_name = skill.get("name", "unknown")
            virtual_path = skill.get("virtual_path", f"/skills/{skill_name}/SKILL.md")
            content = skill.get("content", "")
            files_dict[virtual_path] = content
        return files_dict
...
def load_skills():
    """Load all skills (returns skill objects list)"""
    return get_skill_manager().get_skills()


def get_skill_paths():
    """Get virtual paths for DeepAgents"""
    return get_skill_manager().get_skill_paths()


def get_skills_files_dict():
    """Get files dictionary for DeepAgents"""
    return get_skill_manager().get_skills_files_dict()
